...
local function ListItem(lev, ch)
  ...
  local parser = whitespacechar^0
     * commentchar * (spacechar^2)
     * start^lev
     * #(- start)
     * spacechar^0
     * Ct((V"Inline" - newline)^1)
     * newline^-1
     * ((#(commentchar * spacechar^2) * Ct(subitem("*")^1) / pandoc.BulletList)
        +
        (#(commentchar * spacechar^2) * Ct(subitem("#")^1) / pandoc.OrderedList)
        +
        Cc(nil))
...
G = P{ "Pandoc",
  ...
  ;
  Header = commentchar * spacechar -- There must be a space after #
     ...
     * newline -- and each header ends with a newline
     / function (x) return pandoc.Header(1, x) end -- All headers are level 1
  ;
  Para = commentchar * (spacechar^2) -- There must be at least two spaces after # 
